...
{
  ros::init(argc, argv, "MADYNEGate_KnxClientNode");

  ros::NodeHandle n;
  ros::ServiceClient client = n.serviceClient<knx_service::KnxSet>("MADYNEGate/KnxServerNode/KnxAPI");
  knx_service::KnxSet srv;
 ...
 // srv.request.value = atoll(argv[3]);
  ...
  {
    ROS_INFO("Value of %s = %s", srv.request.target.c_str(), /*atoi(*/srv.response.xml.c_str()/*)*/);
  }
